...
export default function FavoriteTray({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const fetchFavorites = async () => {
    try {
      ...
      const favRes = await fetch(`/api/favorites?email=${sessionData.user.email}`);
      ...
      if (favData.success) {
        ...
        for (const fav of favData.favorites) {
          const carRes = await fetch(`/api/getparticular?name=${encodeURIComponent(fav.carName)}`);
  ...
  const handleRemoveFavorite = async (carName: string) => {
    try {
      ...
      const res = await fetch(
        `/api/favorites?email=${sessionData.user.email}&carName=${encodeURIComponent(carName)}`,
        { method: "DELETE" }
      );
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          {loading ? (
            <div className="flex justify-center items-center min-h-[200px]">
              <LoadingSpinner size="lg" text="Loading your favorite cars..." />
            </div>
          ) : favorites.length === 0 ? (
            <div className="text-center text-gray-500 py-12">
              <p className="text-lg">No favorite cars yet</p>
              <p className="text-sm mt-2">Add cars to your favorites to see them here</p>
            </div>
          ) : (
                        ...
                        {car.powerType && (
                          <div className="flex items-center gap-1">
                            {car.powerType === "Electric" ? (
                              ...
                              <FaLeaf className="text-green-500" title="Hybrid Vehicle" />
                            ) : null}
                          </div>
                        )}
                      </div>
